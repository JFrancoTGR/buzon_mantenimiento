...
const application = 'maintenance';
const locationCode = getLocationCode();
let registeredEmail = '';

if (locationCode && locationContext) {
...
resendButton?.addEventListener('click', async () => {
  ...
  try {
    const payload = await apiRequest('/api/auth/resend-verification', {
      method: 'POST',
      body: JSON.stringify({
        email: registeredEmail,
        application,
...
function showResult(data) {
  ...
  if (data.email_sent === false) {
    ...
    deliveryMessage.textContent = 'Debes confirmar tu correo antes de acceder a Mantenimiento.';
