...
final class PasswordResetService
{
    ...
    public function request(string $email): array
    {
        ...
        $userStatement = $this->pdo->prepare(
            "SELECT id, first_name, last_name, email, password_hash, status, email_verified_at
             FROM users
             WHERE email = :email
             LIMIT 1"
        );
        ...
        if (!$this->isEligibleUser($user)) {
            $this->audit->safeRecord(
                ...
                null,
                null,
                ['email_hash' => $emailHash, 'reason' => 'account_not_eligible']
            );
        ...
        try {
            $this->mailer->sendPasswordResetEmail(
                ...
                (string) $issued['raw_token']
            );

            $this->finalizeDelivery($userId, (int) $issued['token_id'], true);
            $this->audit->safeRecord(
            ...
            $this->finalizeDelivery($userId, (int) $issued['token_id'], false);
            $this->audit->safeRecord(
     ...
     */
    public function reset(string $rawToken, string $password, string $confirmation): array
    {
        ...
        try {
            $this->mailer->sendPasswordResetCompletedNotification(
                $userId,
                $recipientEmail,
                $recipientName
            );
    ...
    private function issueToken(int $userId): ?array
    {
        ...
        try {
            // Serializa solicitudes concurrentes para una misma cuenta.
            ...
            if ($lockUser->fetchColumn() === false || !$this->canIssueForUser($userId)) {
                $this->pdo->commit();
                return null;
            }
    ...
    private function assertIpRateLimit(?string $ipAddress): void
    {
        ...
        $maxPerHour = max(1, Env::int('PASSWORD_RESET_MAX_PER_IP_HOUR', 10));
    ...
    private function canIssueForUser(int $userId): bool
    {
        // El límite por cuenta cuenta entregas reales, no intentos SMTP fallidos.
        // Así una caída temporal de correo no consume innecesariamente el cupo del usuario.
        ...
        $maxPerHour = max(1, Env::int('PASSWORD_RESET_MAX_PER_USER_HOUR', 3));
        if ((int) $countStatement->fetchColumn() >= $maxPerHour) {
            return false;
        }
        ...
        $cooldownMinutes = max(1, Env::int('PASSWORD_RESET_COOLDOWN_MINUTES', 10));
        $latest = new DateTimeImmutable($latestAt, new DateTimeZone('UTC'));
        $now = new DateTimeImmutable('now', new DateTimeZone('UTC'));

        return $latest->add(new DateInterval("PT{$cooldownMinutes}M")) <= $now;
    }

    /** @param mixed $row */
    private function isEligibleUser(mixed $row): bool
    {
        return is_array($row)
    ...
    /** @param mixed $row */
    private function isUsableResetRow(mixed $row): bool
    {
        if (!is_array($row)) {
            return false;
        }
    ...
    private function normalizeToken(string $rawToken): string
    {
        ...
        return hash('sha256', strtolower($rawToken));
    }
